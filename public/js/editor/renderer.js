...
var Renderer = (function () {
    function Renderer(canvas, container, skip_render) {
        //derwish edit
        this.max_zoom = 2;
        this.min_zoom = 0.1;
    ...
    Renderer.prototype.processMouseDown = function (e) {
        ...
        var n = this.container.getNodeOnPos(e.canvasX, e.canvasY, this.visible_nodes);
                ...
                if (!this.connecting_node && !n.flags.collapsed && !this.live_mode) {
                    ...
                    if (n.outputs)
                        for (var o in n.outputs) {
                            ...
                            var link_pos = n.getConnectionPos(false, +o);
    ...
    Renderer.prototype.processMouseUp = function (e) {
        ...
        if (e.which == 1) {
            ...
            if (this.connecting_node) {
                this.dirty_canvas = true;
                this.dirty_bgcanvas = true;
                var node = this.container.getNodeOnPos(e.canvasX, e.canvasY, this.visible_nodes);
                //node below mouse
                if (node) {
    ...
    Renderer.prototype.isOverNodeOutput = function (node, canvasx, canvasy, slot_pos) {
        if (node.outputs)
            for (var o in node.outputs) {
                ...
                var link_pos = node.getConnectionPos(false, +o);
                if (utils_1.default.isInsideRectangle(canvasx, canvasy, link_pos[0] - 10, link_pos[1] - 5, 20, 10)) {
                    if (slot_pos) {
                        slot_pos[0] = link_pos[0];
                        slot_pos[1] = link_pos[1];
    ...
    Renderer.prototype.processDrop = function (e) {
        ...
        var node = this.container.getNodeOnPos(pos[0], pos[1]);
        if (!node) {
            if (this.onDropItem)
    ...
    Renderer.prototype.drawFrontCanvas = function () {
        ...
        if (this.container) {
            ...
            if (this.connecting_pos != null) {
                ...
                var link_color = nodes_1.Nodes.options.NEW_LINK_COLOR;
    ...
    Renderer.prototype.drawNode = function (node, ctx) {
        var glow = false;
        var color = node.color || nodes_1.Nodes.options.NODE_DEFAULT_COLOR;
        ...
        var shape = node.shape || nodes_1.Nodes.options.NODE_DEFAULT_SHAPE;
        ...
        if (!node.flags.collapsed) {
            ...
            if (node.outputs)
                for (var o in node.outputs) {
                    ...
                    var pos = node.getConnectionPos(false, +o);
                    pos[0] -= node.pos[0];
                    pos[1] -= node.pos[1];
    ...
    Renderer.prototype.drawNodeShape = function (node, ctx, size, fgcolor, bgcolor, no_title, selected) {
        ...
        /* gradient test
         ...
         ctx.fillStyle = grad;
         //*/
        var title_height = nodes_1.Nodes.options.NODE_TITLE_HEIGHT;
    ...
    Renderer.prototype.drawNodeCollapsed = function (node, ctx, fgcolor, bgcolor) {
        ...
        ctx.strokeStyle = fgcolor || nodes_1.Nodes.options.NODE_DEFAULT_COLOR;
        ctx.fillStyle = bgcolor || nodes_1.Nodes.options.NODE_DEFAULT_BGCOLOR;
        var collapsed_radius = nodes_1.Nodes.options.NODE_COLLAPSED_RADIUS;
        //circle shape
        var shape = node.shape || nodes_1.Nodes.options.NODE_DEFAULT_SHAPE;
        if (shape == "circle") {
            ctx.beginPath();
        ...
        for (var id in this.container._nodes) {
            ...
            if (node.inputs)
                for (var i in node.inputs) {
                    ...
                    var color = nodes_1.Nodes.options.LINK_TYPE_COLORS[node.inputs[i].type];
    ...
    Renderer.prototype.onMenuAdd = function (node, e, prev_menu, canvas, first_event) {
        ...
        var values = nodes_1.Nodes.getNodeTypesCategories();
        var entries = {};
        for (var i in values)
            if (values[i])
        ...
        function inner_clicked(v, e) {
            ...
            var node_types = nodes_1.Nodes.getNodeTypesInCategory(category);
            var values = [];
    ...
    Renderer.prototype.onMenuNodeColors = function (node, e, prev_menu) {
        var values = [];
        for (var i in nodes_1.Nodes.options.NODE_COLORS) {
            var color = nodes_1.Nodes.options.NODE_COLORS[i];
            var value = {
                value: i,
     ...
     * @param node
     */
    Renderer.prototype.onMenuNodeClone = function (node) {
        if (node.clonable == false)
            return;
        //derwish removed
