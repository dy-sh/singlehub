...
var UiChartNode = (function (_super) {
    __extends(UiChartNode, _super);
    function UiChartNode() {
        _super.call(this, "Chart", template);
        this.dataUpdated = false;
        //client side
        this.CHART_HEIGHT = "190px";
    ...
    UiChartNode.prototype.onAdded = function () {
    ...
    UiChartNode.prototype.startSending = function () {
        var that = this;
    ...
    UiChartNode.prototype.onInputUpdated = function () {
    ...
    ;
    UiChartNode.prototype.onGetMessageToServerSide = function (data) {
        if (data.clear) {
            this.isRecentlyActive = true;
            this.properties['log'] = [];
    ...
    ;
    UiChartNode.prototype.onGetMessageToDashboardSide = function (data) {
        if (data.clear)
            this.dataset.clear();
        if (data.value)
    ...
    ;
    UiChartNode.prototype.createChart = function () {
        var that = this;
    ...
    UiChartNode.prototype.addChartData = function (data) {
        this.dataset.add(data);
        var max = this.settings['maxRecords'].value;
        var unwanted = this.dataset.length - max;
        if (unwanted > 0) {
            var items = this.dataset.get();
            for (var i = 0; i < unwanted; i++) {
                this.dataset.remove(items[i]);
            }
        }
    };
    UiChartNode.prototype.renderStep = function () {
    ...
    UiChartNode.prototype.updateChartStyle = function () {
    ...
    UiChartNode.prototype.redrawChart = function (options) {
        var window = this.graph2d.getWindow();
        options.start = window.start;
        options.end = window.end;
        this.graph2d.destroy();
        this.graph2d = new vis.Graph2d(this.body, this.dataset, options);
    };
    UiChartNode.prototype.showNow = function () {
        var that = this;
    ...
    UiChartNode.prototype.showAll = function () {
        clearTimeout(this.zoomTimer);
        this.settings['autoscroll'].value = "none";
        //   graph2d.fit();
        var start, end;
        if (this.dataset.length == 0) {
            ...
            var min = this.dataset.min('x');
            var max = this.dataset.max('x');
    ...
    UiChartNode.prototype.changeStyle = function () {
        var val;
        switch (this.settings['style'].value) {
            case 'bars':
                val = 'splines';
    ...
    UiChartNode.prototype.onDashboardGetRequest = function (req, res) {
        //render log page
        if (req.params[0] == "/log") {
    ...
    UiChartNode.prototype.onEditorApiGetRequest = function (req, res) {
        //ajax get log
        if (req.params[0] == "/log") {
            res.json(this.properties['log']);
        }
    };
    UiChartNode.prototype.onEditorApiPostRequest = function (req, res) {
        //ajax get log
...
container_1.Container.registerNodeType("ui/chart", UiChartNode);
